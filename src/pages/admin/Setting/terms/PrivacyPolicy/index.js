...
function PrivacyPolicy() {
  const { DatalistPrivacy: listPrivacy, DatalistCountry: listCountry } = data();
  ...
  useEffect(() => {
    const htmlContent = `${listPrivacy.Text_Field}`;
  ...
  const Update = async () => {
    try {
      const res = await terms.putPrivacyPolicy({
        Id: 1,
        Text_Field: editer,
        Country: country,
      });
